...
class EquipmentDamageController extends Controller
{
    public function index(): JsonResponse
    {
        ...
        // Physical equipment units currently marked as damaged / under maintenance (Scoped per office)
        $damagedUnitsQuery = EquipmentUnit::with('equipmentType')
            ->whereIn('status', ['damaged', 'under_maintenance']);

        if (!$isSuperAdmin && $officeId) {
            $damagedUnitsQuery->whereHas('equipmentType', function ($q) use ($officeId) {
                $q->where('office_id', $officeId);
            });
        }

        $damagedUnits = $damagedUnitsQuery->latest()->get();
