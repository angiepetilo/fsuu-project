...
import { CheckCircle, XCircle, Loader2, RefreshCw, AlertCircle, ChevronLeft, ChevronRight, Building } from "lucide-react";
...
export default function EquipmentBorrowings() {
  ...
  const fetchBorrowings = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.get(`/avr-equipment-borrowings?page=${page}`);
      setBorrowings(res.data?.data ?? []);
      setMeta(res.data?.meta ?? null);
    } catch {
      setError("Failed to load equipment borrowings.");
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-medium text-slate-800 tracking-tight">Equipment Borrowing</h1>
          <p className="text-sm text-slate-400 mt-0.5">Manage AVR equipment borrowing requests</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={fetchBorrowings} disabled={loading} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-all shadow-xs disabled:opacity-60">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {["Ref #", "Requestor", "Email", "Purpose", "Date", "Status", "Actions"].map(h => (
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-12 text-slate-400">No equipment borrowings found.</td></tr>
              ) : borrowings.map(b => (
                <tr key={b.id} className="hover:bg-slate-50/60 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs font-bold text-purple-600">{b.reference_code ?? "#" + b.id}</td>
                  ...
                  <td className="px-4 py-3 text-slate-600 text-xs max-w-[160px] truncate">{b.purpose}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      {b.status === "pending" && (
                        <>
                          <button onClick={() => handleAction(b.id, "approve")} disabled={!!actionLoading} className="p-1.5 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-all disabled:opacity-50" title="Approve">
                            {actionLoading === b.id + "-approve" ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle size={14} />}
                          </button>
                          <button onClick={() => handleAction(b.id, "reject")} disabled={!!actionLoading} className="p-1.5 rounded-lg bg-red-50 text-red-700 hover:bg-red-100 transition-all disabled:opacity-50" title="Reject">
