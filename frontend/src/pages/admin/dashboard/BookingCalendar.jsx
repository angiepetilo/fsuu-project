      ...
      <div className="grid grid-cols-7 gap-y-1 gap-x-1 text-center text-[11px] font-semibold">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          if (isFullySelected) {
            ...
            if (details.statusClass) {
              containerStyle = `w-7 h-7 rounded-lg border flex flex-col items-center justify-center mx-auto cursor-pointer hover:opacity-80 transition-all shadow-2xs ${details.statusClass}`;
              dateNumberStyle = details.statusClass.includes("bg-white") ? "text-rose-600 font-bold" : "text-white font-bold";
            } else {
          ...
          return (
            <Tooltip key={day} box={boxInfo}>
              <div className={containerStyle}>
                ...
                {hasEvent && !isFullySelected && !details.statusClass && (
