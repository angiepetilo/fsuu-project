...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const [histRes, vbRes, eqRes, ebRes, dmgRes] = await Promise.all([
  ...
  const getDayDetails = (day) => {
    ...
    const status = bookedOnDate.length >= 2 ? "fully" : "partial";
    return {
      status,
      hasEvent: true,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch">
        ...
        <div className="lg:col-span-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between space-y-3">
          ...
          {topBookedDepartments.length > 0 ? (
            <div className="flex flex-col items-center gap-3 py-1 my-auto w-full">
              {/* Donut Metric */}
              <div className="relative w-20 h-20 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                  ...
                  />
                  {topBookedDepartments.reduce((acc, dept, idx) => {
                    const color = DEPT_COLORS[idx % DEPT_COLORS.length];
                    const pct = dept.pct || 0;
                    if (pct <= 0) return acc;
                    const offset = acc.accumulatedOffset;
                    acc.paths.push(
                      <circle
                        key={idx}
                        cx="18"
                    ...
                    acc.accumulatedOffset += pct;
                    return acc;
                  }, { paths: [], accumulatedOffset: 0 }).paths}
                </svg>
                <div className="absolute flex flex-col items-center justify-center text-center">
                  <span className="text-base font-black text-slate-900 font-mono leading-none">{totalVenueBookings}</span>
                  <span className="text-[8px] font-bold text-slate-500 uppercase mt-0.5">Bookings</span>
                </div>
              </div>
              ...
              <div className="space-y-1.5 w-full">
                {topBookedDepartments.slice(0, 4).map((dept, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center justify-between text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white">
                      <div className="flex items-center gap-2 truncate max-w-[150px]">
                        <span className={`w-2 h-2 rounded-full shrink-0 ${dotBg}`} />
                        <span className="font-bold text-slate-900 text-[11px] truncate">{dept.name}</span>
                      </div>
                      <span className="font-mono font-bold text-slate-800 text-[11px]">
                        {dept.pct}% <span className="text-slate-400 font-normal">({dept.count})</span>
                      </span>
                    </div>
                  );
                })}
