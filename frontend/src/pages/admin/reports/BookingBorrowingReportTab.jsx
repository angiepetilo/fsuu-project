...
export default function BookingBorrowingReportTab({
  ...
  const getStatusBadge = (statusStr) => {
    ...
    if (s === "COMPLETED" || s === "RETURNED" || s === "DONE") {
      return (
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm overflow-hidden">
        <div className="p-5 sm:p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h3 className="text-xl font-black text-slate-900 tracking-tight">Venue Booking Reports</h3>
            ...
            <tbody className="divide-y divide-slate-100 text-xs font-semibold">
              {loading ? (
                ...
                paginatedVenueRecords.map((b, idx) => {
                  ...
                  const status = b.tracking_number?.status || b.status || "PENDING";
                  ...
                  return (
                    <tr key={`rpt-venue-${b.id || idx}`} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-4 px-4 text-center">
                        <button
                          type="button"
                          onClick={() => setSelectedRecord({ type: "venue", data: b })}
                          className="w-8 h-8 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-200/80 inline-flex items-center justify-center transition-all cursor-pointer shadow-2xs"
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-sm overflow-hidden">
        <div className="p-5 sm:p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h3 className="text-xl font-black text-slate-900 tracking-tight">Equipment Borrowing Reports</h3>
            <p className="text-xs text-slate-500 font-semibold mt-0.5">
              Historical borrowing logs and equipment loan status summary
            </p>
          </div>
        </div>
                  ...
                  return (
                    <tr key={`rpt-equip-${eb.id || idx}`} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-4 px-4 text-center">
                        <button
                          type="button"
                          onClick={() => setSelectedRecord({ type: "equipment", data: eb })}
                          className="w-8 h-8 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-200/80 inline-flex items-center justify-center transition-all cursor-pointer shadow-2xs"
                          title="View Record Details"
                        >
                          <Eye size={15} />
