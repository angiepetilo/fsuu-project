...
export default function GeneralLayout() {
  ...
  const getFeatureDetails = (path) => {
    ...
    if (path.includes("/today-reservations")) {
      return {
        title: "Today's Reservations",
        subtitle: "Real-time daily schedule of facility bookings and equipment requisitions."
      };
    }
    if (path.includes("/equipment-release")) {
      return {
        title: "Equipment Release Desk",
        subtitle: "Verify booking references, inspect physical units, and record handover."
      };
    }
    if (path.includes("/equipment-return")) {
      return {
        title: "Equipment Return & Inspection",
        subtitle: "Inspect physical condition of returned units, record damage notes, and confirm returns."
      };
    }
          ...
          <div className={sidebarOpen ? "block" : "block lg:hidden"}>
            ...
            {userMenuOpen && (
              <div className="pt-1.5 border-t border-sidebar-border mt-1 space-y-1">
                <Link
                  to="/general/settings?tab=profile"
