...
export default function EquipmentBorrowDetailModal({
  ...
  const canApprove = isSuperAdmin || hasPermission("equipment_borrowing.approve");
  const canReject = isSuperAdmin || hasPermission("equipment_borrowing.reject");
    ...
    try {
      ...
      if (!isPreUse) {
        Object.entries(normalizedConditions).forEach(([bCode, condVal]) => {
          ...
          if (matchedUnit?.id) {
            api.put(`/general/equipment-units/${matchedUnit.id}`, {
              status: newStatus,
        ...
        <div className="overflow-y-auto flex-1 p-6 text-xs space-y-6">
            ...
            <div className="lg:col-span-6 space-y-4">
              ...
              <div className="bg-slate-50/80 border border-slate-200/90 rounded-2xl p-4 space-y-2.5 shadow-xs">
                ...
                <div className="grid grid-cols-3 py-1 text-xs">
                  ...
                  <span className="col-span-2 font-extrabold text-slate-900">{[selected.first_name, selected.middle_name, selected.last_name, selected.suffix].filter(Boolean).join(" ").trim() || selected.requestor_name || selected.filer_name || selected.requestor || "FSUU Filer"}</span>
                </div>
          ...
          {(isCompleted || (isOngoing && isReadyForPostInspection)) ? (
            ...
            <div className="pt-2 border-t border-slate-200">
              <div className="p-6 bg-slate-50/80 border border-slate-200/90 rounded-2xl text-center space-y-3 flex flex-col items-center justify-center min-h-[160px] shadow-2xs">
                <div className="w-9 h-9 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center">
                  <Play size={18} className="fill-blue-600 ml-0.5" />
                </div>
                <div className="space-y-1">
                  <h5 className="font-extrabold text-slate-900 text-xs">Equipment Currently In-Use</h5>
                  <p className="text-[11px] text-slate-500 font-medium max-w-sm mx-auto">
                    Borrower is currently utilizing the released items. When returned, click below to initiate post-equipment inspection.
                  </p>
                </div>
                <button
                  type="button"
