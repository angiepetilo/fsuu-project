...
export default function VenueBookingDetailModal({
  ...
  const { isSuperAdmin, isStudentAssistant, hasPermission } = usePermissions();
  const canApprove = isSuperAdmin || hasPermission("venue_bookings.approve");
  const canReject = isSuperAdmin || hasPermission("venue_bookings.reject");
  ...
  const [violationOptions, setViolationOptions] = useState(() => {
    ...
    return [
      "Facility or Property Damage",
      "Schedule Overrun or Unauthorized Extension",
      "Equipment Misuse or Unauthorized Relocation",
      "Improper Cleanliness or Waste Disposal",
      "Excessive Noise or Facility Policy Breach",
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get("/general/equipment-types").catch(() => ({ data: [] })),
  ...
  useEffect(() => {
    ...
    if (reqCats && reqCats.length > 0) {
      setOverrideCategories(reqCats.map(c => ({
        category: c.category,
  ...
  const getDisplayStatusText = () => {
    if (isPostInspection) {
      return inspectionStatus === "clean" ? "Satisfactory" : "Policy Violation";
    }
    if (isCompletedOrDamaged) {
      return inspectionStatus === "violation" || currentStatus === "damaged" ? "Policy Violation" : "Completed";
    }
        ...
        for (let uIdx = 0; uIdx < reqQty; uIdx++) {
          ...
          if (bCode) {
            ...
            if (matchedType && matchedType.id) {
              dbUpdatePromises.push(
                api.get(`/general/equipment-types/${matchedType.id}`).then(res => {
                  const curr = res.data;
                  let damageIncr = condChoice === "Damaged" ? 1 : 0;
                  let lostIncr = condChoice === "Lost" ? 1 : 0;
                  
                  return api.put(`/general/equipment-types/${matchedType.id}`, {
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-slate-50/50">
          ...
          {isPending ? (
            ...
            <div className="space-y-4">
              ...
              <div className="w-full">
                <VenueEquipmentChecklist
                  ...
                  setOverrideCategories={setOverrideCategories}
            ...
            <div className="space-y-4">
              ...
              <div className="p-6 bg-white border border-slate-200/90 rounded-2xl text-center space-y-3 flex flex-col items-center justify-center min-h-[160px] shadow-xs">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500 font-medium max-w-md mx-auto">
                    The venue reservation is currently in progress. When the event concludes, proceed with post-event facility and equipment inspection using the button below.
                  </p>
                </div>
              </div>
            </div>
          ) : (
            /* POST-EVENT INSPECTION & HISTORY LOG VIEW (Single Clean Side-by-Side Row) */
            <div className="space-y-4">
              <VenueBookingInfo
                selected={selected}
                formatRealTime={formatRealTime}
              ...
              {/* Single Inspection Row: Left = Equipment Return Inspection | Right = Venue Facility Post Inspection */}
