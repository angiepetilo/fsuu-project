...
import { Save, Camera, KeyRound, Lock, Eye, EyeOff, Loader2, X } from "lucide-react";
...
export default function ProfileConfigTab({ showMsg }) {
  ...
  const handlePassFormSubmit = (e) => {
    e.preventDefault();
    const security = checkPasswordSecurity(passForm.new_password);
    if (!security.isValid) {
      if (!security.hasLength) {
        setPassError("Password must be at least 8 characters long.");
      } else if (!security.hasMixedCase) {
        setPassError("Password must include both uppercase and lowercase letters.");
      } else if (!security.hasAlphaNumeric) {
        setPassError("Password must include both letters and numbers.");
      } else if (security.isDictionaryWord) {
        setPassError("Password is too common. Avoid dictionary words and predictable patterns.");
      } else {
        setPassError("Password does not meet university security criteria.");
      }
      return;
    }
    if (passForm.new_password !== passForm.new_password_confirmation) {
        ...
        <div className="fixed inset-0 bg-black/20 z-[1500] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 border border-slate-200">
            ...
            <form onSubmit={handlePassFormSubmit} className="p-5 space-y-3.5 text-xs font-medium">
              ...
              {/* Password Security Strength & Guidance */}
              <PasswordSecurityStrength password={passForm.new_password} showGuidance={true} />
              ...
              {passForm.new_password.length > 0 && passForm.new_password_confirmation.length > 0 && (
                <div className={`p-2 rounded-lg text-[11px] font-semibold border ${passForm.new_password === passForm.new_password_confirmation ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200"}`}>
                  {passForm.new_password === passForm.new_password_confirmation ? "✓ Passwords match" : "✕ Passwords do not match yet"}
                </div>
              )}
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={passLoading || !checkPasswordSecurity(passForm.new_password).isValid || passForm.new_password !== passForm.new_password_confirmation || !passForm.current_password}
