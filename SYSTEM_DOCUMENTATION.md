...
| `PUSHER_APP_KEY` | `<REDACTED_PUSHER_KEY>` | Pusher public application key |
| `PUSHER_APP_SECRET` | `<REDACTED_PUSHER_SECRET>` | Pusher private application secret |
| `PUSHER_APP_CLUSTER` | `ap1` | Pusher Asia-Pacific server cluster |
| `CLOUDINARY_URL` | `cloudinary://<CLOUDINARY_API_KEY>:<CLOUDINARY_API_SECRET>@<CLOUD_NAME>` | Cloudinary persistent media storage (venues, gear, endorsements) |
...
| `VITE_PUSHER_APP_KEY` | `<REDACTED_PUSHER_KEY>` | Public Pusher key for live Laravel Echo connection |
